...
class Pretokenizer:
    ...
    def pretokenize(self, doc: str) -> dict[SymbolSeq, int]:
    ...
    def _string2tuple_of_bytes(self, s: str) -> SymbolSeq:
...
def consecutive_merging(bytepair_counts: dict[SymbolSeq, int], num_merges: int) -> list[SymbolSeq]:
    list_merged_bytes = []
    for _ in tqdm(range(num_merges), desc="Merging bytes"):
        bytepair_counts, merged_bytes = merge_bytes(bytepair_counts)
        list_merged_bytes.append(merged_bytes)
    return list_merged_bytes


def merge_bytes(token2counts: dict[SymbolSeq, int]) -> dict[SymbolSeq, int]:
...
def token_count_to_bpe(token2counts: dict[SymbolSeq, int]) -> dict[SymbolSeq, int]:
...
def most_frequent_conseq_token(conseq_token_counts: dict[SymbolSeq, int]) -> SymbolSeq:
...
def update_token2counts(token2counts: dict[SymbolSeq, int], to_merge: SymbolSeq):
...
def merge_in_new_token(token: SymbolSeq, to_merge: SymbolSeq):
    out, i = [], 0
    while i < len(token):
...
def _run_train_bpe(
...
) -> tuple[dict[int, SymbolSeq], list[SymbolSeq]]:
    ...
    bytepair_counts = parallel_pretokenizer(chunks)

    new_bytepairs = consecutive_merging(bytepair_counts, vocab_size - 256 - len(special_tokens))

    vocab_len = len(vocab)
    for i, new_bytepair in enumerate(new_bytepairs):
        vocab[vocab_len + i] = b"".join(new_bytepair)
    return vocab, new_bytepairs
